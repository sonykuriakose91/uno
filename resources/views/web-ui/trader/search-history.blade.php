    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                ...
                <p><a href="{{ route('ui-home') }}">Home</a><img src="{{ asset('ui/images/arrow-right.svg') }}"> Search History </p>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="row">
                        <table class="table table-bordered datatable">
                            ...
                            <tbody>
                                ...
                                @foreach($searchhistory as $key => $history)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td>{{ ($history->user_type == "customer")?"Your profile was appeared on search by " .$history->getcustomer->name ." (".ucfirst($history->user_type).")" ." on ".date('d-m-Y h:i A',strtotime($history->created_at)):"Your profile was appeared on search by " .$history->getfromtrader->name ." (Trader)" ." on ".date('d-m-Y h:i A',strtotime($history->created_at)) }}</td>
                                </tr>
                                @endforeach
                                @endif
                            </tbody>
