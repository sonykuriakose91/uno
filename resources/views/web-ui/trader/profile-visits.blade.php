    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                ...
                <p><a href="{{ route('ui-home') }}">Home</a><img src="{{ asset('ui/images/arrow-right.svg') }}"> Profile Visits </p>
            </div>
        </div>
    </div>
</div>
                ...
                <div class="trader-dashboard">
                    <div class="row">
                        <table class="table table-bordered datatable">
                            ...
                            <tbody>
                                @if(count($profile_visits) > 0)
                                @foreach($profile_visits as $key => $profile)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td><?= ($profile->user_type == "customer")?$profile->getcustomer->name ." (".ucfirst($profile->user_type).")" ." visited your profile on ".date('d-m-Y h:i A',strtotime($profile->created_at)):$profile->getfromtrader->name ." (Trader)" ." visited your profile on ".date('d-m-Y h:i A',strtotime($profile->created_at)) ?></td>
                                </tr>
                                @endforeach
                                @endif
                            </tbody>
